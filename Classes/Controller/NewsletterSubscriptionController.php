 ...
 */
class NewsletterSubscriptionController extends \Ecom\EcomToolbox\Controller\ActionController {
...
#		$this->settings['mail']['carbonCopy'] = $this->settings['mail']['carbonCopy'] ?: 'marketing@ecom-ex.com ecom Marketing';
		...
		if ( in_array($this->request->getControllerActionName(), [ 'create', 'confirm', 'delete', 'resendActivationMail' ]) ) {
		...
		if ( $url || !$newSubscription instanceof \S3b0\EcomNewsletterSubscription\Domain\Model\Subscription ) {
		  ...
		  // Avoid duplicates, check for existing records
			...
			$this->forward('new', null, null, [
				'newSubscription' => $newSubscription,
				'dismissibleAlert' => $subscription->isHidden() ? $this->getStandAloneTemplate('Notifications/NotConfirmed', [ 'subscription' => $subscription ]) : $this->getStandAloneTemplate('Notifications/DuplicateFound')
			]);
		} else {
	...
	public function confirmAction($hash, $email) {
		...
		if ( $subscription instanceof \S3b0\EcomNewsletterSubscription\Domain\Model\Subscription && (time() - $subscription->getCreationDate() < 604800) ) {
			...
			$mailToSender->setFrom($this->noReplyEmailAddresses ?: $this->senderEmailAddresses)
				...
				->setSubject($this->settings['mail']['senderSubject'] ?: LocalizationUtility::translate('mail_subject_generic', $this->extensionName) . LocalizationUtility::translate('mail_subject_confirmed', $this->extensionName))
	...
	public function deleteAction($hash, $email) {
		...
		if ( $subscription instanceof \S3b0\EcomNewsletterSubscription\Domain\Model\Subscription ) {
			...
			$mailToSender->setFrom($this->noReplyEmailAddresses ?: $this->senderEmailAddresses)
				->setTo([ $subscription->getEmail() => $subscription->getName() ])
				->setSubject($this->settings['mail']['senderSubject'] ?: LocalizationUtility::translate('mail_subject_generic', $this->extensionName) . LocalizationUtility::translate('mail_subject_deleted', $this->extensionName))
	...
	public function initializeResendActivationMailAction() {
		...
		if ( $this->request->hasArgument('subscription') ) {
			$this->request->setArgument('subscription', $this->subscriptionRepository->findByUid((int) $this->request->getArgument('subscription')));
		}
	}
